type IconName = 'moon' | 'sun' | 'search' | 'github' | 'external' | 'branch' | 'chevron';
...
export function Icon({ name }: { name: IconName }) {
  return (
    <svg aria-hidden="true" className="icon" fill={name === 'branch' ? 'currentColor' : 'none'} stroke={name === 'branch' ? 'none' : 'currentColor'} strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" viewBox="0 0 24 24">
      <path d={paths[name]} />
    </svg>
  );
}
